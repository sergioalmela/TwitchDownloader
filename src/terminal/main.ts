...
const handler = require('../main/twitch/handler.controller')
...
async function downloadVod (): Promise<any> {
  ...
  const credentials = await VodController.getAuth(id, true)

  const feeds = await VodController.getFeeds(id, credentials)
