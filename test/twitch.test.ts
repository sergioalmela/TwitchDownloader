...
import Playlist from '../src/main/infrastructure/types/Playlist'
...
import { AuthClipUseCase } from '../src/main/application/useCases/authClip.usecase'
...
const authClipUseCase = container.get<AuthClipUseCase>(ContainerSymbols.AuthClipUseCase)
...
describe('Download feed', async () => {
  const downloadVideo = downloadFromFeedUseCase.execute(urlVod, pathEmpty, fileEmpty, new ExtensionVo(FileExtensions.MP4))
  const downloadAudio = downloadFromFeedUseCase.execute(urlVod, pathEmpty, fileEmpty, new ExtensionVo(FileExtensions.MP3))
